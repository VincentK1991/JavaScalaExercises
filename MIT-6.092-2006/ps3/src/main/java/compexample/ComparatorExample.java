...
/**
 * A GUI for a book of names, emails, phone numbers, and
 * favorite colors.
 * @author rayhe
 *
 * This file is an address-book-like program that allows you to add people
 * to it and delete people from it. It lets you then edit the values
 * for anyone in the table. String values are editable by clicking on the cell and using a keyboard
 * while color are editable by clicking on them. this is the class to run in order to start the GUI
...
public class ComparatorExample{
	...
	/**
	 * Constructor creates the GUI, loading the default table data, and creates
	 * all the containers/components contained by the GUI.
	 */
	...
	/**
	 * loadExamplePeople loads people into the database directly, with
	 * method calls
	 *
	 */
	private void loadExamplePeople(){
		people.add(new Person("Quiche","Lorraine","617-253-1000","quiche@scotty.mit.edu",Color.RED,this));
		people.add(new Person("Alice","Whacker","617-253-7788","aw@scotty.mit.edu",Color.RED.darker().darker(),this));
		people.add(new Person("Zygorthian","Space-Raiders","617-253-1541","zsr@scotty.mit.edu",new Color(50,70,90),this));
		people.add(new Person("Opus","","617-253-1000","opus@scotty.mit.edu",Color.RED.darker(),this));
		people.add(new Person("Rick","Chang","617-232-3257","rick@mit.edu",Color.RED.darker().darker().darker(),this));
		people.add(new Person("Ben","Bitdiddle","303-494-4774","bitdiddle@scotty.mit.edu",new Color(56,50,50),this));
		
	}
	/**
	 * Adds a person, called by the add button, basically
	 * adds a "blank" person, who can then be editted
	 *
	 */
	...
	/**
	 * sorts the list using Collections.sort, the list
	 * of people, and the PeopleComparator written by you!
	 *
	 */
	...
	/**
	 * Used by person (taking a callback and the default dialog
	 * color) to ask the user for a new color, which it then sets
	 * by calling back person
	 * @param p A valid object of class Person
	...
	/** The data model for the table. This class
	 * controls how things are displayed, set, etc.
	 * @author rayhe
	 *
	 */
	private class PersonTableModel extends AbstractTableModel{
		...
		/**
		 * 
		 * @param people - the list from ComparatorExample
		 * for inquiries/to keep in sync
		 */
		...
		/**
		 * relies on Person class to determine number
		 * of fields, what they say and what they do when
		 * edited.
		 * @param columnIndex the column, to match up with Person.getFieldColumn(the index)
		 * @return either the string that matches the column in person.getFieldColumn
		 *	or an empty string (shouldn't happen).
		 */
	...
	public static void main(String[] args) {
		ComparatorExample item = new ComparatorExample();
		item.sort();
	}
}
