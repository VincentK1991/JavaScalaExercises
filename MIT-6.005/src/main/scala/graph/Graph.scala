...
object Graph {
   ...
   */
  def empty[L] = new ConcreteEdgesGraph[L] //throw new RuntimeException("not implemented")
...
trait Graph[L] {
   ...
   *         edge
   */
  def set(source: L, target: L, weight: Int):Int
   ...
   *         the key to target
   */
  def sources(target: L):mutable.Map[L,Int]
   ...
   *         source to the key
   */
  def targets(source: L):mutable.Map[L,Int]
}